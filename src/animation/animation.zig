...
test "AnimationPlayer with custom enum" {
    const TestAnim = enum {
        spell_cast,
        potion_drink,
    };

    var player = AnimationPlayer(TestAnim).init(std.testing.allocator);
    defer player.deinit();
    ...
    try std.testing.expectEqual(@as(u32, 6), anim.total_frames);
}

test "AnimationPlayer with default types" {
    var player = DefaultAnimationPlayer.init(std.testing.allocator);
    defer player.deinit();

    try player.registerAnimation(.walk, 8);
    const anim = player.createAnimation(.walk);

    try std.testing.expectEqual(components.DefaultAnimationType.walk, anim.anim_type);
    try std.testing.expectEqual(@as(u32, 8), anim.total_frames);
}
...
test "generateSpriteName with custom enum" {
    ...
    const name = generateSpriteName(&buffer, "wizard", TestAnim.cast_fireball, 2);
    try std.testing.expectEqualStrings("wizard/cast_fireball_0003", name);
}

test "generateSpriteNameNoPrefix" {
    const TestAnim = enum { teleport };

    var buffer: [64]u8 = undefined;
    const name = generateSpriteNameNoPrefix(&buffer, TestAnim.teleport, 0);
    try std.testing.expectEqualStrings("teleport_0001", name);
}
